-- tinyint : 1바이트 사용 -128~127까지의 정수 표현 가능 (unsigined시 => 0~255)
...
-- 정상적으로 insert
insert into author(id,name,email,height) values(7, '홍길동3', 'sss@naveer.com', 175.5);

-- 데이터가 잘리도록 insert
insert into author(id,name,email,height) values(7, '홍길동3', 'sss@naveer.com', 175.55);
...
-- blob(바이너리 데이터) 실습
-- 일반적으로 blob으로 저장하기보다는, 이미질를 별도로 저장하고, 이미지 경로를 varvhar로 저장
...
insert into author(id, name,email,birthday) values()
...
-- regexp : 정규표현식을 활용한 조회 (미리 약속되어서 시스템에 저장된 문법)
select *from author where name regexp '[a-z]';   -- 이름에 알파벳 소문자가 포함된 경우
select *from author where name regexp '[가-힣]';   -- 이름에 한글 포함된 경우

-- 타입변환 - cast
-- 문자 -> 숫자
select cast('12' as unsigned);
-- 숫자 -> 날짜
select cast(200251121 as date);  -- 2025-11-21
-- 문자 -> 날짜
...
select * from post where created_time >= '2025-11-01' and created_time < '2025-11-20';
                     자동적으로 2025-11-01 00:00:00 ~ 2025-11-20 00:00:00 까지로 조회